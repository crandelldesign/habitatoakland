...
class NavWalker extends \Walker_Nav_Menu {
  ...
  function start_el( &$output, $item, $depth = 0, $args = [], $id = 0 ) {
    ...
    if ( $item->is_subitem ) {
        $link_btn = '';
        $dropdown_toggle = '<a href="#" role="button" class="nav-link dropdown-toggle dropdown-toggle-split" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="sr-only">Toggle Dropdown</span></a>';
        $item_html = str_replace( '<a', ' '.$link_btn.'<a class="nav-link"', $item_html );
        $item_html = str_replace( '</a>', ' </a>'.$dropdown_toggle, $item_html );
    } else {
